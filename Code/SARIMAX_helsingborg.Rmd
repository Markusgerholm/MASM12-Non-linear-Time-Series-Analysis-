...
y_all1 <- ts(df_cc$y, frequency = 24)
...
i_block_start <- i_test1_start - maxLagX
i_block_end   <- i_test1_end
df_test_block <- df_full[i_block_start:i_block_end, ]
keep <- complete.cases(df_test_block$y, df_test_block[, X_cols])
df_test_block_cc <- df_test_block[keep, ]
y_test_block <- ts(df_test_block_cc$y, frequency = 24)
X_test_block <- as.matrix(df_test_block_cc[, X_cols])
```
...
```{r}
yhat_block <- fitted(fit_block)

# Identify which rows of df_test_block_cc correspond to the far test window timestamps
test_ts <- df_full$ts_utc[i_test1_start:i_test1_end]
idx_test <- which(df_test_block_cc$ts_utc %in% test_ts)

# Safety: should be h if nothing was dropped
length(idx_test)

y_obs_test_sarimax <- df_test_block_cc$y[idx_test]
yhat_sarimax_test  <- as.numeric(yhat_block)[idx_test]
```

```{r}
fit_block_base <- Arima(y_test_block, model = fit)  # reuse baseline SARIMA coefficients
yhat_block_base <- fitted(fit_block_base)

yhat_sarima_test <- as.numeric(yhat_block_base)[idx_test]
rmse <- function(a,b) sqrt(mean((a-b)^2, na.rm=TRUE))
mae  <- function(a,b) mean(abs(a-b), na.rm=TRUE)

cat("n test points:", length(idx_test), "\n")
cat("SARIMA RMSE:", rmse(y_obs_test_sarimax, yhat_sarima_test),
    " MAE:", mae(y_obs_test_sarimax, yhat_sarima_test), "\n")
cat("SARIMAX RMSE:", rmse(y_obs_test_sarimax, yhat_sarimax_test),
    " MAE:", mae(y_obs_test_sarimax, yhat_sarimax_test), "\n")
