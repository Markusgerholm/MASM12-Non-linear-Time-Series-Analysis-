...
## Skillinge & Falsterbo - element lags
```{r}
par(mfrow=c(1,2))
plot_ccf_with_prewhitened_station(
  df_he = df_he,
...
## Köbenhavn lufthavn & Roskilde lufthavn - element lag
```{r}
par(mfrow=c(1,2))
plot_ccf_with_prewhitened_station(
  df_he = df_he,
...
## Roskilde lufthavn & Gniben - element lag
```{r}
par(mfrow=c(1,2))
plot_ccf_with_prewhitened_station(
  df_he = df_he,
...
## Sletterhage fyr & Anholt - element lag
```{r}
par(mfrow=c(1,2))
plot_ccf_with_prewhitened_station(
  df_he = df_he,
...
```
##Ljungby - element lag vs hourly lag
```{r}
par(mfrow=c(1,2))
plot_ccf_with_prewhitened_station(
  df_he = df_he,
...
rank_stations_by_ccf_lags <- function(df_he, station_list, reg,
  ...
  # baseline CV
  base <- score_arimax_fixed(y, X = NULL, initial = initial)
...
rank_R1 <- rank_stations_by_ccf_lags(df_he, stations_R1, reg = "R1",
                                     lag_max = 50, top_k = 3, initial = 400)
rank_R1
```
...
rank_R2 <- rank_stations_by_ccf_lags(df_he, stations_R2, reg = "R2",
...
rank_R4 <- rank_stations_by_ccf_lags(df_he, stations_R4, reg = "R4",
                                     lag_max = 50, top_k = 3, initial = 200)
rank_R4
```

## Winners based on RMSE
